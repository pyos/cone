...
cold_defn(unsigned, sleep, unsigned sec) {
    return !cone ? cold_call(sleep, sec) : cone_sleep(cot_u128_mul((cot_nsec){0, sec}, 1000000000ul)) ? sec : 0;
}

cold_defn(int, nanosleep, const struct timespec *req, struct timespec *rem) {
    return !cone ? cold_call(nanosleep, req, rem) : cone_sleep(cot_nsec_from_timespec(*req));
}

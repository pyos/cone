#include "romp.h"

enum romp_signo
{
    ROMP_SIGN_NONE   = 0,     // signature ::= typesign*
    ROMP_SIGN_UINT   = 'u',   // typesign ::= 'u' {octets :: digit}
    ROMP_SIGN_INT    = 'i',   //            | 'd' {octets :: digit}
    ROMP_SIGN_DOUBLE = 'f',   //            | 'f'
    ROMP_SIGN_VEC    = 'v',   //            | 'v' {type :: typesign}
    ROMP_SIGN_STRUCT = '(',   //            | '(' {contents :: signature} ')';
};

struct romp_sign
{
    enum romp_signo sign;
    unsigned size;
    unsigned align;
};

struct romp_nested_vec mun_vec(struct mun_vec);

// Decode a single element from a signature, advancing the pointer to the next one.
// If the returned element is a vector, the next element is the type of its contents.
// On error, returns a zero-initialized structure.
//
// Errors: `romp_sign_syntax` if the signature is invalid.
//
static struct romp_sign romp_sign(const char **sign) {
    struct romp_sign r = {};
next:
    switch ((*sign)[0]) {
        case ' ':
            (*sign)++;
            goto next;
        case 0:
        case ')':
            mun_error(romp_sign_syntax, "expected a type");
            break;
        case ROMP_SIGN_DOUBLE:
            r = (struct romp_sign){*(*sign)++, 8, _Alignof(double)};
            break;
        case ROMP_SIGN_VEC:
            r = (struct romp_sign){*(*sign)++, sizeof(struct mun_vec), _Alignof(struct mun_vec)};
            break;
        case ROMP_SIGN_INT:
        case ROMP_SIGN_UINT:
            switch ((*sign)[1]) {
                case '1': r = (struct romp_sign){*(*sign)++, *(*sign)++  - '0', _Alignof(uint8_t)};  break;
                case '2': r = (struct romp_sign){*(*sign)++, *(*sign)++  - '0', _Alignof(uint16_t)}; break;
                case '4': r = (struct romp_sign){*(*sign)++, *(*sign)++  - '0', _Alignof(uint32_t)}; break;
                case '8': r = (struct romp_sign){*(*sign)++, *(*sign)++  - '0', _Alignof(uint64_t)}; break;
                default: mun_error(romp_sign_syntax, "`%c' is not a valid int size", (*sign)[1]); break;
            }
            break;
        case ROMP_SIGN_STRUCT:
            r.sign = *(*sign)++;
            const char *sc = *sign;
            for (struct romp_sign s; (s = romp_sign(&sc)).sign;) {
                r.size += s.size;
                r.align = r.align > s.align ? r.align : s.align;
            }
            break;
        default:
            mun_error(romp_sign_syntax, "`%c' is not a known type", (*sign)[0]);
    }
    return r;
}

static int romp_encode_uint(struct romp *out, uint64_t in, unsigned width) {
    uint8_t packed[] = { in >> 56, in >> 48, in >> 40, in >> 32, in >> 24, in >> 16, in >> 8, in };
    return mun_vec_extend(out, &packed[8 - width], width) MUN_RETHROW;
}

static int romp_decode_uint(struct romp *in, uint64_t *out, unsigned width) {
    if (in->size < width)
        return mun_error(romp, "need %u octets, have %u", width, in->size);
    *out = width == 1 ? romp_r1(in->data)
         : width == 2 ? romp_r2(in->data)
         : width == 4 ? romp_r4(in->data)
         : width == 8 ? romp_r8(in->data) : 0;
    mun_vec_erase(in, 0, width);
    return 0;
}

static int romp_encode_int(struct romp *out, int64_t in, int width) {
    return romp_encode_uint(out, (uint64_t)in, width);
}

static int romp_decode_int(struct romp *in, int64_t *out, int width) {
    uint64_t u = 0;
    if (romp_decode_uint(in, &u, width) MUN_RETHROW)
        return -1;
    *out = u >> 63 ? -(int64_t)~u - 1 : (int64_t)u;
    return 0;
}

static int romp_encode_double(struct romp *out, double in) {
    union { double f; uint64_t d; } u = { .f = in };
    return romp_encode_uint(out, u.d, 8);
}

static int romp_decode_double(struct romp *in, double *d) {
    union { double f; uint64_t d; } u = {.d = 0};
    if (romp_decode_uint(in, &u.d, 8) MUN_RETHROW)
        return -1;
    *d = u.f;
    return 0;
}

static int romp_encode_vec(struct romp *out, const char **sign, const struct mun_vec *in) {
    struct romp_sign s = romp_sign(sign);
    if (!s.sign || romp_encode_uint(out, in->size, 4) MUN_RETHROW)
        return -1;
    if (s.sign == ROMP_SIGN_VEC) {
        const char *signreset = *sign;
        const struct romp_nested_vec *v = (const struct romp_nested_vec *) in;
        for mun_vec_iter(v, nv)
            if (*sign = signreset, romp_encode_vec(out, sign, nv) MUN_RETHROW)
                return -1;
        return 0;
    }
    return mun_vec_extend(out, in->data, in->size * s.size) MUN_RETHROW;
}

static int romp_decode_vec(struct romp *in, const char **sign, struct mun_vec *out) {
    uint64_t size = 0;
    struct romp_sign s = romp_sign(sign);
    if (!s.sign || romp_decode_uint(in, &size, 4) MUN_RETHROW)
        return -1;
    if (mun_vec_reserve_s(s.size, out, size) MUN_RETHROW)
        return -1;
    if (s.sign == ROMP_SIGN_VEC) {
        struct romp_nested_vec *v = (struct romp_nested_vec *) out;
        for (const char *signreset = *sign; size--; ) {
            mun_vec_append(v, &(struct mun_vec){});
            if (*sign = signreset, romp_decode_vec(in, sign, &v->data[v->size - 1]) MUN_RETHROW)
                return mun_vec_fini(v), -1;
        }
    } else
        mun_vec_extend_s(s.size, out, in->data, size);
    mun_vec_erase(in, 0, s.size * size);
    return 0;
}

#define REALIGNED_TO(ptr, i) \
    (ptr = ((uintptr_t)ptr & (i - 1)) ? (__typeof__(ptr))(((uintptr_t)ptr + i) & ~(i - 1)) : ptr)

#define REALIGNED_AS(ptr, T) ((T*)REALIGNED_TO(ptr, _Alignof(T)))

static int romp_encode_struct(struct romp *out, const char **sign, const char *in, int nested) {
    int64_t ir = 0;
    uint64_t ur = 0;
    for (struct romp_sign s; **sign != (nested ? ')' : 0);) switch ((s = romp_sign(sign)).sign) {
        case ROMP_SIGN_NONE:
            return -1;
        case ROMP_SIGN_UINT:
            ur = s.size == 1 ? *REALIGNED_AS(in, const uint8_t)
               : s.size == 2 ? *REALIGNED_AS(in, const uint16_t)
               : s.size == 4 ? *REALIGNED_AS(in, const uint32_t)
               : s.size == 8 ? *REALIGNED_AS(in, const uint64_t) : 0;
            if (romp_encode_uint(out, ur, s.size) MUN_RETHROW)
                return -1;
            in += s.size;
            break;
        case ROMP_SIGN_INT:
            ir = s.size == 1 ? *REALIGNED_AS(in, const int8_t)
               : s.size == 2 ? *REALIGNED_AS(in, const int16_t)
               : s.size == 4 ? *REALIGNED_AS(in, const int32_t)
               : s.size == 8 ? *REALIGNED_AS(in, const int64_t) : 0;
            if (romp_encode_int(out, ir, s.size) MUN_RETHROW)
                return -1;
            in += s.size;
            break;
        case ROMP_SIGN_DOUBLE:
            if (romp_encode_double(out, *REALIGNED_AS(in, const double)) MUN_RETHROW)
                return -1;
            in += s.size;
            break;
        case ROMP_SIGN_VEC:
            if (romp_encode_vec(out, sign, REALIGNED_AS(in, const struct mun_vec)) MUN_RETHROW)
                return -1;
            in += s.size;
            break;
        case ROMP_SIGN_STRUCT:
            if (romp_encode_struct(out, sign, REALIGNED_TO(in, s.align), 1) MUN_RETHROW)
                return -1;
            in += s.size;
            break;
    }
    if (nested)
        (*sign)++;
    return 0;
}

static int romp_decode_struct(struct romp *in, const char **sign, char *out, int nested) {
    int64_t ir = 0;
    uint64_t ur = 0;
    for (struct romp_sign s; **sign != (nested ? ')' : 0);) switch ((s = romp_sign(sign)).sign) {
        case ROMP_SIGN_NONE:
            return -1;
        case ROMP_SIGN_UINT:
            if (romp_decode_uint(in, &ur, s.size) MUN_RETHROW)
                return -1;
            switch (s.size) {
                case 1: *REALIGNED_AS(out, uint8_t ) = ur; break;
                case 2: *REALIGNED_AS(out, uint16_t) = ur; break;
                case 4: *REALIGNED_AS(out, uint32_t) = ur; break;
                case 8: *REALIGNED_AS(out, uint64_t) = ur; break;
            }
            out += s.size;
            break;
        case ROMP_SIGN_INT:
            if (romp_decode_int(in, &ir, s.size) MUN_RETHROW)
                return -1;
            switch (s.size) {
                case 1: *REALIGNED_AS(out, int8_t ) = ir; break;
                case 2: *REALIGNED_AS(out, int16_t) = ir; break;
                case 4: *REALIGNED_AS(out, int32_t) = ir; break;
                case 8: *REALIGNED_AS(out, int64_t) = ir; break;
            }
            out += s.size;
            break;
        case ROMP_SIGN_DOUBLE:
            if (romp_decode_double(in, REALIGNED_AS(out, double)) MUN_RETHROW)
                return -1;
            out += s.size;
            break;
        case ROMP_SIGN_VEC:
            if (romp_decode_vec(in, sign, REALIGNED_AS(out, struct mun_vec)) MUN_RETHROW)
                return -1;
            out += s.size;
            break;
        case ROMP_SIGN_STRUCT:
            if (romp_decode_struct(in, sign, REALIGNED_TO(out, s.align), 1) MUN_RETHROW)
                return -1;
            out += s.size;
            break;
    }
    if (nested)
        (*sign)++;
    return 0;
}

int romp_encode_var(struct romp *out, const char *sign, va_list args) {
    int64_t ir = 0;
    uint64_t ur = 0;
    for (struct romp_sign s; *sign;) switch ((s = romp_sign(&sign)).sign) {
        case ROMP_SIGN_NONE:
            return -1;
        case ROMP_SIGN_UINT:
            ur = s.size == 4 ? va_arg(args, uint32_t)
               : s.size == 8 ? va_arg(args, uint64_t)
               : va_arg(args, unsigned);
            if (romp_encode_uint(out, ur, s.size) MUN_RETHROW)
                return -1;
            break;
        case ROMP_SIGN_INT:
            ir = s.size == 4 ? va_arg(args, int32_t)
               : s.size == 8 ? va_arg(args, int64_t)
               : va_arg(args, unsigned);
            if (romp_encode_int(out, ir, s.size) MUN_RETHROW)
                return -1;
            break;
        case ROMP_SIGN_DOUBLE:
            if (romp_encode_double(out, va_arg(args, double)) MUN_RETHROW)
                return -1;
            break;
        case ROMP_SIGN_VEC:
            if (romp_encode_vec(out, &sign, va_arg(args, const struct mun_vec *)) MUN_RETHROW)
                return -1;
            break;
        case ROMP_SIGN_STRUCT:
            if (romp_encode_struct(out, &sign, va_arg(args, const char *), 1) MUN_RETHROW)
                return -1;
            break;
    }
    return 0;
}

int romp_decode_var(struct romp *in, const char *sign, va_list args) {
    int64_t ir = 0;
    uint64_t ur = 0;
    for (struct romp_sign s; *sign;) switch ((s = romp_sign(&sign)).sign) {
        case ROMP_SIGN_NONE:
            return -1;
        case ROMP_SIGN_UINT:
            if (romp_decode_uint(in, &ur, s.size) MUN_RETHROW)
                return -1;
            switch (s.size) {
                case 1: *va_arg(args, uint8_t  *) = ur; break;
                case 2: *va_arg(args, uint16_t *) = ur; break;
                case 4: *va_arg(args, uint32_t *) = ur; break;
                case 8: *va_arg(args, uint64_t *) = ur; break;
            }
            break;
        case ROMP_SIGN_INT:
            if (romp_decode_int(in, &ir, s.size) MUN_RETHROW)
                return -1;
            switch (s.size) {
                case 1: *va_arg(args, int8_t  *) = ir; break;
                case 2: *va_arg(args, int16_t *) = ir; break;
                case 4: *va_arg(args, int32_t *) = ir; break;
                case 8: *va_arg(args, int64_t *) = ir; break;
            }
            break;
        case ROMP_SIGN_DOUBLE:
            if (romp_decode_double(in, va_arg(args, double *)) MUN_RETHROW)
                return -1;
            break;
        case ROMP_SIGN_VEC:
            if (romp_decode_vec(in, &sign, va_arg(args, struct mun_vec *)) MUN_RETHROW)
                return -1;
            break;
        case ROMP_SIGN_STRUCT:
            if (romp_decode_struct(in, &sign, va_arg(args, char *), 1) MUN_RETHROW)
                return -1;
            break;
    }
    return 0;
}

int romp_encode(struct romp *out, const char *sign, ...) {
    va_list args;
    va_start(args, sign);
    int ret = romp_encode_var(out, sign, args);
    va_end(args);
    return ret;
}

int romp_decode(struct romp *in, const char *sign, ...) {
    va_list args;
    va_start(args, sign);
    int ret = romp_decode_var(in, sign, args);
    va_end(args);
    return ret;
}

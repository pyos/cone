...
static struct cone_event_fd_sub **cone_event_fd_bucket(struct cone_event_fd *set, int fd) {
    struct cone_event_fd_sub **b = &set->fds[fd % (sizeof(set->fds) / sizeof(set->fds[0]))];
    while (*b && (*b)->fd != fd) b = &(*b)->link;
    return b;
}

static struct cone_event_fd_sub *cone_event_fd_open(struct cone_event_fd *set, int fd) {
...
static void cone_event_fd_close(struct cone_event_fd *set, struct cone_event_fd_sub *ev) {
#if CONE_EPOLL
...
struct cone *cone_incref(struct cone *c) {
    c->refcount++;
    return c;
}
...
int cone_main(size_t stksz, struct cone_closure body) {
    struct cone_loop loop = {};

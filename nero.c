...
static int nero_writer(struct nero *n) {
    ...
    while (n->wbuffer.size) {
        ...
        if ((size = write(n->fd, data, size)) < 0)
            return mun_error_os();
...
static int nero_write(struct nero *n, const uint8_t *data, size_t size) {
    if (!n->writer && !(n->writer = cone(&nero_writer, n)))
        return mun_error_up();
    if (mun_vec_extend(&n->wbuffer, data, size))
        return mun_error_up();
    return mun_ok;
}
...
static int nero_write_request(struct nero *n, uint32_t rqid, const char *function, struct romp *args) {
    ...
    if (nero_write_header(n, NERO_FRAME_REQUEST, rqid, nlen + args->size))
        return mun_error_up();
    if (nero_write(n, (const uint8_t*)function, nlen))
        return mun_error_up();
    return nero_write(n, args->data, args->size);
}
...
static int nero_write_response_error(struct nero *n, uint32_t rqid) {
    ...
    if (nero_write_header(n, NERO_FRAME_RESPONSE_ERROR, rqid, nlen + tlen + 4))
        return mun_error_up();
    return nero_write(n, buf, nlen + tlen + 4);
}
...
static int nero_restore_error(const uint8_t *data, size_t size, const char *function) {
    ...
    mun_error_at(code, "nero_remote", "<remote>", function, 0, "%s: %s", name, text);
    return mun_error_up();
}
...
static int nero_call_wait(struct nero *n, struct nero_future *fut, const char *function, va_list args) {
    ...
    switch (fut->rr) {
        ...
        case NERO_RETURN_CANCEL:
            return mun_error(cancelled, "nero stopping");
    }
...
int nero_run(struct nero *n) {
    ...
    for (ssize_t size; (size = read(n->fd, data, sizeof(data))); ) {
        ...
        if (mun_vec_extend(&n->rbuffer, data, size))
            return mun_error_up();
        while (n->rbuffer.size >= 8) {
            ...
            if (n->rbuffer.size < size + 8)
                break;
            if (nero_on_frame(n, read1(n->rbuffer.data), read4(n->rbuffer.data + 4), n->rbuffer.data + 8, size))
                return mun_error_up();
...
int nero_call_var(struct nero *n, const char *function, va_list args) {
    ...
    if (mun_vec_append(&n->queued, &fp))
        return mun_error_up();
    if (nero_call_wait(n, &fut, function, args)) {
        mun_vec_fini(&fut.data);
        for (unsigned i = 0; i < n->queued.size; i++)
            if (n->queued.data[i] == &fut)
                return mun_vec_erase(&n->queued, i, 1), mun_error_up();
        return mun_error_up();
    }
    mun_vec_fini(&fut.data);
    return mun_ok;

...
static int test_siy_primitive() {
    ...
    if (siy_encode(&out, "i2 u8 f", &x) MUN_RETHROW)
        return -1;
    if (out.size != 18)
        return mun_error(assert, "2 + 8 + 8 = 18, not %u", out.size);
    if (siy_decode(&out, "i2 u8 f", &y) MUN_RETHROW)
        return -1;
    CHECK_FIELD(x, y, a, "%d");
...
static int test_siy_vec() {
    ...
    if (siy_encode(&out, "vi1", &x) || siy_decode(&out, "vi1", &y) MUN_RETHROW)
        return -1;
...
static int test_siy_vec_struct() {

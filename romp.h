...
static inline int romp_encode_uint(struct romp_iovec *out, uint64_t in, unsigned width) {
    ...
    if (width != 1 && width != 2 && width != 4 && width != 8)
        return veil_error(assert, "invalid integer width");
...
#define X(i) ((uint64_t)in->data[i])
    switch (width) {
        ...
        default: return veil_error(assert, "invalid uint size %u", width);
    }
#undef X
...
static inline int romp_encode_vec(struct romp_iovec *out, const char **sign, const struct veil_vec *in) {
    struct romp_sign s = romp_sign(sign);
    if (!s.sign)
        return veil_error_up();
    if (s.sign == romp_sign_vec)
        return veil_error(assert, "vectors of vectors not implemented");  // TODO serialize vectors of vectors
    if (romp_encode_uint(out, in->size, 4) || veil_vec_extend(out, in->data, in->size * s.stride))
        return veil_error_up();
    return veil_ok;
}

static inline int romp_decode_vec(struct romp_iovec *in, const char **sign, struct veil_vec *out) {
    uint64_t size;
    struct romp_sign s = romp_sign(sign);
    if (!s.sign)
        return veil_error_up();
    if (s.sign == romp_sign_vec)
        return veil_error(assert, "vectors of vectors not implemented");  // TODO deserialize vectors of vectors
    veil_vec_fini_s(s.stride, out);
    if (romp_decode_uint(in, &size, 4) || veil_vec_splice_s(s.stride, out, 0, in->data, size))
        return veil_error_up();
